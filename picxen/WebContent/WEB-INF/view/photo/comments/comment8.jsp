...
<script type="text/javascript">
	function postComment(){
	 ...
	 var totlog = request.getParameter("plBean");
	 
	 system.out.println(totlog);
	 	 	/* for( h=0;h<) */
...
<!-- 탑 추천 -->						
						...
						<c:choose>
							...
							<c:when test="${mcvBean.userid == userid || clikeBean.userid == userid }"> <!-- 틀림 맞음 -->
							...
							<c:when test="${mcvBean.userid != userid || clikeBean.userid != userid}">	<!-- 틀림 -->
							...
							<c:if test="${mcvBean.commentNo!=clikeBean.commentNo}">
								<c:set var="lnt" value="0"/>
								<c:forEach var="h" begin="0" end="${totalCmlog-1}" >
										<c:set var="lcnt" value="${lnt+1 }" />
										<c:set var="clikeBean" value="${cmlog[h] }" />
										<c:if test="${mcvBean.commentNo==clikeBean.commentNo}" >
											<input type="text" id ="logCmNo${lnt}" value="${clikeBean.commentNo}"/>	
											<input type="text" id ="logCmUsrId${lnt}" value="${clikeBean.userid}"/>			
