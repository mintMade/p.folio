...
<!-- hashMap -->
	...
	<input type="text" name="totlog1" value="<%=request.getAttribute("cmlog") %>"/>
	 ...
	 <%
	 	...
	 	List<CmLikeBean> totlog = (List<CmLikeBean>)request.getAttribute("cmlog");
	    
	 	int totsize = (Integer)request.getAttribute("totalCmlog");
	 	int lnt = 0;
	 	for(int l=0;l<totsize;l++){
	 		if(totsize<1)break;
	 		CmLikeBean cliBean = totlog.get(l);
	 		int comNo = cliBean.getCommentNo();
	 		lnt++;
	 	}
